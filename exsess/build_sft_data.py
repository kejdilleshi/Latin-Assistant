...
#
# Requires:
#   pip install openai>=1.40.0 pydantic>=2.5
...
class TransformItem(BaseModel):
    type: str = Field(description="e.g., 'Active↔Passive', 'Simplify-ResultClause'")
    source_latin: str
    target_latin: str
...
- translation: literal_en and idiomatic_en
- morphosyntax: UD-style tokens (form, lemma, upos, feats, head, deprel). Head is 1-based; 0 for root.
- constructions: short labels like ["Hortatory-Subjunctive","Causal-Quoniam","Foreign-Greek-Term"]
- transforms: 1–2 simple rewrites (e.g., voice flip; result-clause simplification)
- minimal_pairs: 1–2 pairs (good vs. near-miss) with a single controlled error and an error_tag.

Rules:
- Keep Greek text inside <GRC>…</GRC> unchanged.
- Literal translation can be stiff; idiomatic should read natural English.
...
def make_sft_items(
    ...
    base_meta = {
        ...
        "phenomena": list(pack.constructions),
    }
    pid = f"p{paragraph_idx:04d}.s{sentence_idx:02d}"

    items = []

    # 1) Translation (literal)
    items.append({
        "id": f"{pid}.t.translate_literal",
        "task": "translate_literal",
        "prompt": f"Task: Translate the Latin sentence into English (literal). Latin: {sentence_text}",
        "target": pack.translation.literal_en.strip(),
        "meta": base_meta,
    })

    # 2) Translation (idiomatic)
    items.append({
        "id": f"{pid}.t.translate_idiomatic",
        "task": "translate_idiomatic",
    ...
    # 4) Construction tagging
    items.append({
        "id": f"{pid}.c.tags",
        "task": "construct_tagging",
        "prompt": f"Task: List all constructions present as a JSON array of short labels. Latin: {sentence_text}",
        "target": json.dumps(pack.constructions, ensure_ascii=False),
        "meta": base_meta,
    })
    ...
    # 6) Minimal pairs (limit)
    for i, mp in enumerate(pack.minimal_pairs[:max_minpairs], start=1):
        prompt = (
            "Task: Two Latin variants follow. Select the correct one and name the error in the other. "
            f"A) {mp.good} B) {mp.bad}"
        )
        target = f"Correct: A. Error in B: {mp.error_tag}."
        items.append({
            "id": f"{pid}.m.{i:02d}",
            "task": "contrast_judge",
            "prompt": prompt,
            "target": target,
            "meta": {**base_meta, "error_tag": mp.error_tag},
        })
...
def main():
    ...
    ap.add_argument("--max_transforms", type=int, default=1, help="Max transforms per sentence")
    ap.add_argument("--max_minpairs", type=int, default=1, help="Max minimal pairs per sentence")
    args = ap.parse_args()
    ...
    try:
        for p_idx, rec in enumerate(subset, start=1):
            paragraph = rec["text"]
            ...
            try:
                ...
                print(f"[WARN] Segmentation validation failed for paragraph {p_idx}: {ve}", file=sys.stderr)
                continue
            ...
            for s_idx, sent in enumerate(seg.sentences, start=1):
                ...
                try:
                    ...
                    print(f"[WARN] Teacher pack failed at p={p_idx}, s={s_idx}: {e}", file=sys.stderr)
                    continue
                ...
                items = make_sft_items(
                    ...
                    max_minpairs=args.max_minpairs,
                )
                for it in items:
